/* Mouse-tracked 3D tilt for timeline cards, family-tree nodes, and the
   vote's team cards. Uses event delegation on document so it works on
   elements rendered after this script loads. Skipped entirely on touch
   devices (no hover) and when the user prefers reduced motion. */
(function () {
  const SELECTOR = ".timeline-card, .node, .poll-team";
  const MAX_DEG = 7;
