/* Mouse-tracked 3D tilt for family-tree nodes. Uses event delegation on
   document so it works on elements rendered after this script loads.
   Skipped entirely on touch devices (no hover) and when the user prefers
   reduced motion. */
(function () {
  const SELECTOR = ".node";
  const MAX_DEG = 7;
